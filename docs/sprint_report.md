...
# Milestone: Persistent evaluation run tracking

Goal: persist each tutor evaluation (run + per-case results) so quality can be compared
over time across providers (mock → openai → anthropic), models, prompt versions, and
retrieval versions. Backend-only. Mock stays default; no paid APIs.

## Why this matters before enabling real LLM providers

You cannot tell whether OpenAI/Anthropic (or a fine-tune, or a new prompt) is *better*
without a recorded baseline to compare against. This milestone records the mock baseline
and an audit trail (score, per-case failures, git sha, notes) so any future provider change
is measured, not guessed.

## What changed

- **Models (`models.py`):** added `EvaluationRun` and `EvaluationCaseResult`. No secrets are
  stored; only an `answer_preview` (≤300 chars), not full answers. `EvaluationCaseResult`
  links to `AIInteraction` when available.
- **Migration:** `0004_evaluationrun_evaluationcaseresult.py` (additive; generated by Django).
- **Admin:** both models registered; `EvaluationRun` shows case results inline.
- **Evaluator service (`evaluation/tutor_evaluator.py`):** `evaluate_tutor_cases(...)` now
  accepts `save`, `notes`, `metadata`, `fail_under`; computes `duration_ms`, `provider`,
  `model_name`, `passed_threshold`; returns `evaluation_run_id` (None when not saved).
  `evaluate_case` now always carries `answer_preview` + `diagnostics` + `model_name` so a run
  can be persisted without `--verbose`. Persistence is one atomic transaction; git sha is
  best-effort.
- **Command (`evaluate_tutor`):** added `--save/--no-save` (default **save**), `--notes`,
  `--list-runs`, `--show-run <id>`. `--json` includes `evaluation_run_id`. Threshold failures
  still persist the run, then exit nonzero.
- **Tests:** +9 (model creation ×2, save creates run+cases, no-save creates none, JSON has
  run id, list-runs, show-run, threshold-failure still stored, save uses no paid clients).
- **Docs:** README "Persistent evaluation run tracking" section; this entry.
...
| `manage.py test backend.exam_intelligence` | **39 tests OK** (1 pgvector test skipped on SQLite) |
| `evaluate_tutor --verbose --save` | 8/8, score 1.000, `evaluation_run_id: 1`, RESULT: PASS |
| `evaluate_tutor --json --save` | JSON includes `evaluation_run_id` |
| `evaluate_tutor --no-save` | `evaluation_run_id: (not saved)`, no row written |
| `evaluate_tutor --list-runs` | lists saved runs |
| shell inspection | `runs=2`, `case_results=16`, last score 1.0, last passed True |

## Not verified here

- The real PostgreSQL path (vector search, pgvector smoke check) — unchanged this milestone
  and still the acceptance gate on the dev machine. Evaluation persistence itself is
  backend-agnostic and was verified on SQLite.
- No OpenAI/Anthropic calls were made or enabled (a test patches both clients to raise if
  instantiated during a saved evaluation).

## Recommended next milestone

Provider comparison reporting: a `compare_eval_runs <id_a> <id_b>` command (or
